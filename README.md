...
**Цель:** Проверить, что поверхностный классификатор (rule-based + ML) может заменить LLM для роутинга сообщений.

**Критерии успеха:**
- F1 >= 0.85 для классификации intent/scenario
- Снижение LLM-вызовов >= 30% по сравнению с baseline
- Качество маршрутизации не ухудшается

**Классы intent:**
- `booking` — запись на приём
- `complaint_primary` — первичная жалоба
- `followup_question` — уточняющий вопрос
- `price_question` — вопрос о цене
- `reschedule_cancel` — перенос/отмена
- `other` — прочее
...
**Критерии успеха:**
- Заполнение обязательных полей >= 90% (vs 60-70% baseline)
- Увеличение длины диалога не более +1-2 сообщения
- >= 70% диалогов "достаточны" по экспертной оценке
...
**Критерии успеха:**
- Улучшение качества поиска (Precision/Recall/F1) на контрольных кейсах
- Стабильность результатов при неоднозначных формулировках вопросов
- Прозрачность и воспроизводимость отчётов по стратегиям
